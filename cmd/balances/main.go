...
func LogBalances(ctx context.Context, binance binance.Interface, config *configfile.ConfigFile) {
...
func LogUSDTValue(ctx context.Context, binance binance.Interface, config *configfile.ConfigFile) {
	b, err := binance.GetBalance(ctx, append(config.Coins, config.Bridge)...)
	if err != nil {
		fmt.Println("Failed to get balances:", err.Error())
		return
